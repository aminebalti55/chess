...
bashDB_URL=jdbc:postgresql://aws-1-eu-north-1.pooler.supabase.com:5432/postgres?sslmode=require
...
Install & run
bashcd frontend/chess-client
npm install
ng serve
App available at: http://localhost:4200
🔐 Authentication Flow (JWT)

Register: POST /api/auth/register → { token, userId, displayName }
Login: POST /api/auth/login → { token, userId, displayName }
Current user: GET /api/users/me (with Authorization: Bearer <token>)
...
Start backend, then start frontend
Register two users (e.g., in Chrome and Firefox, or Chrome normal vs Incognito)
Both login → /lobby lists 2 online users
User A clicks Invite on User B → User B receives a dialog to accept/decline
On accept, both are routed to /game/:id. Moves are synced in real-time
Reload either tab — the game resumes (moves fetched from DB; live WS resumes)
...
bashDB_URL=jdbc:postgresql://<region>.pooler.supabase.com:5432/postgres?sslmode=require
